...
std::unordered_map<IDType, std::pair<float, IDType>> Dijkstra(WeightedGraph const &graph, IDType const start, IDType const end)
{
    ...
    while (!to_visit.empty())
    {
        auto const [cout_actuel, id_actuel] = to_visit.top();
        to_visit.pop();

        if (cout_actuel > distances[id_actuel].first)
        {
            continue;
        }
        ...
        for (auto const &arête : graph.get_neighbors(id_actuel))
        {

            float nouveau_cout = cout_actuel + static_cast<float>(arête.weight);

            if (distances.find(arête.to) == distances.end() || nouveau_cout < distances[arête.to].first)
            {

                distances[arête.to] = {nouveau_cout, id_actuel};

                to_visit.push({nouveau_cout, arête.to});
            }
        }
    }
